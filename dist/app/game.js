...
class Canvas {
    constructor(tileSize, nrTilesX, nrTilesY) {
    ...
    drawBite(bite) {
        if (bite.position.x < this.nrTilesX && bite.position.y < this.nrTilesY) {
            ...
            this.context.beginPath();
            this.context.roundRect(bite.position.x * this.tileSize, bite.position.y * this.tileSize, this.tileSize, this.tileSize, [10]);
            this.context.stroke();
            this.context.fill();
        }
    }
    drawSnake(snake) {
        snake.path.forEach((anchor) => {
            ...
            this.context.fillRect(anchor.x * this.tileSize, anchor.y * this.tileSize, this.tileSize, this.tileSize);
        });
    }
    clear() {
        this.context.clearRect(0, 0, this.nrTilesX * this.tileSize, this.nrTilesY * this.tileSize);
        this.drawLines();
    }
}
/* init game */
...
const run = () => {
    ...
    function update() {
        canvas.clear();
        bite.set(Math.floor(Math.random() * MAX_X), Math.floor(Math.random() * MAX_Y));
        canvas.drawBite(bite);
        canvas.drawSnake(snake);
    }
